...
export default function Home(){
  ...
  async function runAnalysis(){
    setRunning(true);
    try{
      const res=await fetch('/api/analyze',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({period,sensor,variables})});
  ...
  return <div className="shell">
    ...
    <main className="main">
      ...
      <div className="metricgrid"><Metric label="AOI" value="Ready"/><Metric label="Period" value={`${period.start}–${period.end}`}/><Metric label="Variables" value="12"/><Metric label="Outputs" value="Q1 Pack"/></div>
      {tab==='Publication Maps'?<PublicationMaps mapType={mapType} setMapType={setMapType} layout={layout} setLayout={setLayout} dpi={dpi} setDpi={setDpi}/>:tab==='Paper Report'?<PaperReport period={period} sensor={sensor} result={result}/>:<AnalysisWorkspace tab={tab} period={period} setPeriod={setPeriod} sensor={sensor} setSensor={setSensor} result={result}/>} 
    </main>
  </div>
}
...
function AnalysisWorkspace({tab,period,setPeriod,sensor,setSensor,result}:any){
  return <div className="grid">
    <section className="card"><div className="mapframe"><div className="maplabel">Interactive AOI / Analysis Map</div><div className="mapcoords">Basemap + GEE raster layers</div><div className="northmini">N ↑</div><div className="scalebar">0&nbsp;&nbsp;5&nbsp;&nbsp;10 km</div></div></section>
...
function moduleText(tab:string){
  const map:Record<string,string>={
    'Map & AOI':'Draw or upload AOI and reference ecosystem polygons. The AOI geometry will be reused in all maps, statistics and paper exports.',
...
function PublicationMaps({mapType,setMapType,layout,setLayout,dpi,setDpi}:any){
  ...
  return <div className="pub"><div className="pubgrid"><section className="card"><div className="sectionhead"><div><h3>Publication Layout Builder</h3><p>Journal-ready cartography for study area and result figures</p></div><span className="badge">{layout}</span></div>
    <div className="layoutpreview" ref={mapRef}>
      ...
      <div className="metadata">CRS: EPSG:4326 • Source: Google Earth Engine • Scale: analysis dependent</div>
...
function PaperReport({period,sensor,result}:any){
  ...
  async function exportDocx(){
    ...
    const blob=await Packer.toBlob(doc); downloadBlob(blob,'Q1_Methods_Results_Draft.docx');
  }

  async function exportPdf(){
    const {jsPDF}=await import('jspdf'); const pdf=new jsPDF({unit:'mm',format:'a4'}); let y=16; const margin=16; const width=178;
    pdf.setFontSize(14); pdf.text('Beyond Greening — Q1 Methods & Results Report',margin,y); y+=10; pdf.setFontSize(11); pdf.text('2. Materials and Methods',margin,y); y+=7; pdf.setFontSize(9);
    const m=pdf.splitTextToSize(methods,width); pdf.text(m,margin,y); y+=m.length*4+7; pdf.setFontSize(11); pdf.text('3. Results',margin,y); y+=7; pdf.setFontSize(9); const r=pdf.splitTextToSize(results,width); pdf.text(r,margin,y); pdf.save('Q1_Methods_Results_Report.pdf');
  }

  return <div className="reportgrid"><section className="card report" ref={reportRef}><div className="paperkicker">AUTOMATED PAPER REPORT</div><h2>Beyond Greening: Decoupling Vegetation, Moisture, and Thermal Recovery Trajectories in Tropical Post-Mining Landscapes</h2><h3>2. Materials and Methods</h3><h4>2.1 Study Design and Data Processing</h4><p>{methods}</p><h3>3. Results</h3><p>{results}</p><h4>Planned paper tables</h4><ol><li>Data sources and spatial resolution</li><li>Descriptive statistics</li><li>Mann–Kendall and Sen’s slope results</li><li>Correlation matrix</li><li>Regression coefficients</li><li>Validation metrics</li></ol><h4>Planned paper figures</h4><ol><li>Study area map</li><li>NDVI–NDMI–LST spatial patterns</li><li>Annual recovery trajectories</li><li>Sen’s slope / trend significance</li><li>Correlation heatmap</li><li>PCA biplot</li><li>Driver-response relationships</li><li>Reference-ecosystem comparison</li></ol></section><aside className="card"><div className="form"><div className="field"><label>Analysis status</label><div className="statusbox">{result?.status||'Awaiting connected GEE analysis'}</div></div><div className="field"><label>Paper package</label><div className="hint">Exports Methods and Results with consistent variable names, analysis provenance, tables, figure captions and map assets. Numerical results are inserted only after backend execution.</div></div><button onClick={exportDocx}>Export Methods + Results (.docx)</button><button onClick={exportPdf}>Export Report (.pdf)</button><button className="secondary" onClick={()=>alert('Q1 package will bundle DOCX/PDF, tables, publication maps and scientific raster exports once the GEE backend is connected.')}>Generate Q1 Paper Package</button></div></aside></div>
}
